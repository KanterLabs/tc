#!/usr/bin/env python3
"""Private, retry-safe local state for the TC Roadmap Codex hooks.
...
def _atomic_write(path: Path, state: SessionState) -> None:
    ...
    fd = -1
    try:
        fd, temp_name = tempfile.mkstemp(prefix=".tc-roadmap-", suffix=".tmp", dir=path.parent)
        os.fchmod(fd, 0o600)
...
def warning(exc: BaseException) -> str:
    ...
    return f"tc-roadmap: local session state unavailable ({type(exc).__name__}); continuing"
