...
def reconcile_hooks(target: Path) -> object:
    ...
    if install_hooks is None:
        raise UpdateError("the installed TC Roadmap skill has no lifecycle hook installer")
...
def _reconcile_hooks_safely(target: Path) -> None:
    ...
    try:
        ...
        print(
            "tc-roadmap update: warning: lifecycle hooks were not reconciled; "
    ...
    if getattr(result, "changed", False) is True:
        print(
            "tc-roadmap update: lifecycle hooks changed; review and trust them in Codex /hooks "
