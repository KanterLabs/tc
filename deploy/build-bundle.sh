...
SHA=${1:-}
SIGNING_KEY_FILE=${ROADMAP_RELEASE_SIGNING_KEY_FILE:-}
CLOUDFLARED_TOKEN_FILE=${ROADMAP_CLOUDFLARED_TOKEN_FILE:-"$DIST_DIR/cloudflared.token"}
OWNER_ENV_FILE=${ROADMAP_OWNER_ENV_FILE:-"$DIST_DIR/owner.env"}
...
safe_file "$DIST_DIR/roadmap" roadmap-binary
[[ -x "$DIST_DIR/roadmap" ]] || {
	printf 'roadmap binary must be executable\n' >&2
	exit 1
}
[[ -n "$SIGNING_KEY_FILE" ]] || {
	printf 'ROADMAP_RELEASE_SIGNING_KEY_FILE is required to sign a release\n' >&2
	exit 1
}
...
install -m 0755 "$DIST_DIR/roadmap" "$BUNDLE_DIR/roadmap"
...
install -m 0755 "$ROOT_DIR/deploy/roadmap-backup.sh" "$BUNDLE_DIR/roadmap-backup.sh"
install -m 0755 "$ROOT_DIR/deploy/roadmap-restore.sh" "$BUNDLE_DIR/roadmap-restore.sh"
install -m 0755 "$ROOT_DIR/deploy/roadmap-rollback.sh" "$BUNDLE_DIR/roadmap-rollback.sh"
install -m 0644 "$ROOT_DIR/deploy/roadmap-backup.service" "$BUNDLE_DIR/roadmap-backup.service"
install -m 0644 "$ROOT_DIR/deploy/roadmap-backup.timer" "$BUNDLE_DIR/roadmap-backup.timer"
install -m 0644 "$ROOT_DIR/deploy/roadmap.service" "$BUNDLE_DIR/roadmap.service"
...
if grep -q '^ROADMAP_RELEASE_SHA=' "$BUNDLE_DIR/roadmap.env"; then
	printf 'owner environment already contains ROADMAP_RELEASE_SHA\n' >&2
	exit 1
fi
printf 'ROADMAP_RELEASE_SHA=%s\n' "$SHA" >> "$BUNDLE_DIR/roadmap.env"
...
ARCHIVE="$DIST_DIR/roadmap-$SHA.tar.gz"
rm -f -- "$ARCHIVE"
