#!/usr/bin/env python3
"""Install the Helm lifecycle hooks without changing other hooks."""
...
HOOK_RELATIVE_PATH = Path("skills") / "helm" / "scripts" / "helm_hook.py"
HOOK_MODE = 0o600
HOOK_TIMEOUTS = {
    "SessionStart": (5, "Refreshing Helm session"),
    "PostToolUse": (30, "Refreshing Helm liveness"),
    "PreCompact": (30, "Refreshing Helm liveness"),
    "Stop": (5, "Refreshing Helm session"),
}
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        if result.changed:
            ...
            message = "no-op; Helm lifecycle hooks are already installed"
        json.dump(
            {
        ...
        print(f"helm hooks: {exc}", file=sys.stderr)
        return 1
